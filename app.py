...
def rjust_filter(s, width, fillchar=' '):
    """Custom rjust filter for Jinja2 templates"""
...
def init_db():
    ...
    # Create products table - FIXED THE SYNTAX ERROR
...
def send_email_otp(email, otp):
    try:
        ...
        msg.body = f'Your OTP for verification is: {otp}. It will expire in 10 minutes.'
        mail.send(msg)
        return True
    except Exception as e:
...
def send_sms_otp(phone, otp):
    try:
        if twilio_client is None:
            print("Twilio client not initialized. SMS sending disabled.")
            return False
...
def register():
    if request.method == 'POST':
        ...
        if existing_user:
            ...
            if existing_user['phone_verified'] == 0:
                conn.execute('DELETE FROM users WHERE id = ?', (existing_user['id'],))
                conn.execute('DELETE FROM otps WHERE email = ?', (email,))
                conn.commit()
                flash('Previous unverified registration removed. Please register again.', 'info')
            else:
                flash('Email or phone number already registered!', 'danger')
                conn.close()
                return render_template('register.html')
        else:
            flash('New registration detected.', 'info')
        ...
        # Try SMS first, if fails use email
        sms_success = send_sms_otp(phone, otp)
        ...
        if sms_success:
            ...
            if send_email_otp(email, otp):
                session['verify_email'] = email
                flash('SMS unavailable. OTP sent to your email instead!', 'warning')
                return redirect(url_for('verify_otp', purpose='register'))
            else:
...
def resend_otp():
    if request.method == 'POST':
        ...
        if user and user['phone_verified'] == 0:
            ...
            # Send OTP
            sms_success = send_sms_otp(user['phone'], otp)
